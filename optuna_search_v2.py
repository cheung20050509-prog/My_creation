...
INT_BOUNDS = {
    "stage1_epochs": (3, 20),
    "ema_start_epoch": (3, 10),
    "num_infogate_layers": (2, 5),
}
...
def suggest_tier2(trial, local_space=None):
    return {
        ...
        "weight_decay": suggest_float_param(
            ...
            local_space=local_space),
        "ema_decay": suggest_float_param(
            trial, "ema_decay", LINEAR_FLOAT_BOUNDS["ema_decay"], log=False,
            local_space=local_space),
    }
...
def suggest_tier3(trial, local_space=None):
    return {
        ...
        "ema_start_epoch": suggest_int_param(
            trial, "ema_start_epoch", INT_BOUNDS["ema_start_epoch"], local_space),
    }
...
def apply_dataset_bounds_overrides(dataset):
    ...
    if dataset == "mosi":
        ...
        # Bottleneck categorical kept as superset of top-32 so warm-start trials are not dropped.
        ...
        INT_BOUNDS["num_infogate_layers"]    = (3, 5)
...
def build_local_search_space_from_trials(
    ...
    *,
    ...
    tier1_cat_names = ("bottleneck_dim",)
    tier1_int_names = ("num_infogate_layers",)
    ...
    for name in tier1_int_names:
        vals = values_for(name)
        if vals:
            local_space[name] = narrow_int_range(
                vals, INT_BOUNDS[name], min_pad=stage1_min_pad)

    if search_tier >= 2:
        tier2_log_names = ("alpha_ib", "weight_decay")
        tier2_linear_names = ("warmup_proportion", "ema_decay")
    ...
    if search_tier >= 3:
        ...
        tier3_int_names = ("ema_start_epoch",)
        ...
        for name in tier3_cat_names:
            ...
            if vals:
                local_space[name] = ordered_choice_subset(
                    CATEGORICAL_SPACE[name], vals, best_params.get(name))
        for name in tier3_int_names:
            vals = values_for(name)
            if vals:
                local_space[name] = narrow_int_range(
                    vals, INT_BOUNDS[name], min_pad=1)
...
def objective(trial, cli):
    ...
    _esp = int(getattr(cli, "early_stop_patience", 0) or 0)
    if _esp > 0:
        cmd.extend(["--early_stop_patience", str(_esp)])
        _esmd = float(getattr(cli, "early_stop_min_delta", 0.0) or 0.0)
        if _esmd > 0.0:
            cmd.extend(["--early_stop_min_delta", str(_esmd)])
...
def main():
    ...
    pa.add_argument(
        ...
        type=int,
        default=0,
        help="Forwarded to train.py when >0: stop after this many dev epochs "
             "(epoch>=stage1) without selection-metric improvement; 0 disables.",
    )
    pa.add_argument(
        "--early_stop_min_delta",
        type=float,
        default=0.0,
        help="Forwarded to train.py with --early_stop_patience when >0.",
    )
