...
class InfoGateLayer(nn.Module):
    ...
    """
    def __init__(self, hidden_dim, num_heads=4, dropout=0.1, num_aux=2, ablation='none'):
        super().__init__()
        assert num_aux in (2, 3), f"InfoGateLayer: num_aux must be 2 or 3, got {num_aux}"
        self.num_aux = num_aux
        self._abl_no_adaptive_gate = (ablation == 'no_adaptive_gate')
    ...
    def forward(self, B_p, conf_p, B_aux_list, conf_aux_list,
        ...
        if self._abl_no_adaptive_gate:
            B_p_fused = B_p_up
            for ca_out in ca_outs:
                B_p_fused = B_p_fused + self.dropout(ca_out)
        else:
...
class InfoGateModule(nn.Module):
    ...
    def __init__(self, hidden_dim, num_layers=3, num_heads=4, dropout=0.1, num_aux=2, ablation='none'):
        super().__init__()
        self.num_aux = num_aux
        self.layers = nn.ModuleList([
            InfoGateLayer(hidden_dim, num_heads, dropout, num_aux=num_aux, ablation=ablation)
...
class InfoGate(nn.Module):
    ...
    def __init__(self, args):
        ...
        # --- 2b. no_ib bypass projections ---
        if self.ablation == 'no_ib':
            self.no_ib_proj_t = nn.Linear(unified_dim, bn_dim)
            self.no_ib_proj_a = nn.Linear(unified_dim, bn_dim)
            self.no_ib_proj_v = nn.Linear(unified_dim, bn_dim)
            if self.use_hcf:
                self.no_ib_proj_h = nn.Linear(unified_dim, bn_dim)
        ...
        self.infogate = InfoGateModule(bn_dim, num_layers, num_heads, dropout,
                                        num_aux=num_aux, ablation=self.ablation)
    ...
    def forward(self, text, acoustic, visual, hcf=None, labels=None, stage=1,
        ...
        # 2. IB encode ------------------------------------------------
        if self.ablation == 'no_ib':
            B_t = self.no_ib_proj_t(F_t)
            B_a = self.no_ib_proj_a(F_a)
            B_v = self.no_ib_proj_v(F_v)
            mu_t, lv_t = B_t, torch.zeros_like(B_t)
            mu_a, lv_a = B_a, torch.zeros_like(B_a)
            mu_v, lv_v = B_v, torch.zeros_like(B_v)
            conf_t = torch.ones(B_t.shape[0], B_t.shape[1], 1, device=device, dtype=B_t.dtype)
            conf_a = torch.ones_like(conf_t)
            conf_v = torch.ones_like(conf_t)
        else:
        ...
        if self.use_hcf:
            if self.ablation == 'no_ib':
                B_h = self.no_ib_proj_h(F_h)
                mu_h, lv_h = B_h, torch.zeros_like(B_h)
                conf_h = torch.ones(B_h.shape[0], B_h.shape[1], 1, device=device, dtype=B_h.dtype)
            else:
        ...
        if self.ablation == 'no_mselector':
            # Fix primary to text modality (index 1 in modality_order: a=0, t=1, v=2, h=3)
            text_idx = 1
            num_mod = len(self.modality_order)
            weights = F.one_hot(torch.full((B['t'].shape[0],), text_idx, device=device, dtype=torch.long),
                                num_classes=num_mod).float()
            primary_onehot = weights.clone()
            primary_idx = torch.full((B['t'].shape[0],), text_idx, device=device, dtype=torch.long)
        else:
            if self.use_hcf:
        ...
        if self.ablation == 'no_infogate':
            ...
            # no_conf_gating: strip confidence → standard attention
            cp = None if self.ablation == 'no_conf_gating' else conf_p
            ca = [None] * len(conf_aux_list) if self.ablation == 'no_conf_gating' else conf_aux_list
            B_p_enhanced = self.infogate(B_p, cp, B_aux_list, ca, tok_mask)
