...
class InfoGateLayer(nn.Module):
    ...
    def forward(self, B_p, conf_p, B_aux_list, conf_aux_list,
        ...
        for ca, Bn, c in zip(cas, B_aux_n, conf_aux_list):
            kconf = None if self.no_conf_gating else c
            ca_outs.append(ca(B_p_n, Bn, Bn, kconf, tok_mask))

        # IB-guided self-attention on primary
        conf_sa = None if self.no_conf_gating else conf_p
        ...
        B_aux_out_list = []
        conf_back = None if self.no_conf_gating else conf_p
        for ca_p, ffn, ln_ff, Bn, B_orig in zip(ca_ps, ffns, ln_ffs, B_aux_n, B_aux_list):
            ca_p_aux = ca_p(Bn, B_p_fn, B_p_fn, conf_back, tok_mask)
...
class InfoGate(nn.Module):
    ...
    def _fixed_language_primary(self, Bs, device, num_modalities):
        """Force language (`t`) as primary; indices follow ``self.modality_order``."""
        idx_t = self.modality_order.index('t')
        primary_idx = torch.full((Bs,), idx_t, device=device, dtype=torch.long)
        primary_onehot = F.one_hot(primary_idx, num_classes=num_modalities).float()
        weights = primary_onehot.clone()
        return weights, primary_onehot, primary_idx
    ...
    def _ib_bypass_forward(self, F_dict):
        ...
        for m in self.modalities:
            x = F_dict[m]
            B[m] = self.ib_bypass[m](x)
            mu[m] = torch.zeros_like(B[m])
            lv[m] = torch.zeros_like(B[m])
            conf[m] = torch.ones_like(B[m])
        return B, mu, lv, conf
