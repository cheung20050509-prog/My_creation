#!/usr/bin/env bash
# SIMSv2 tier3_fresh trial 53: baseline + three core PRISM ablations.
# Multi-GPU: default = one training process per GPU per wave; next wave after
# the current wave completes. Override JOBS_PER_GPU for multi-job-per-card.
#
# Baseline (--ablation none) certification vs fixed_experiment reproduce:
#   fixed_experiment/runs/simsv2_tier3_fresh_trial53/train.log
# For strict reproduction, run none as the sole CUDA job on that GPU.
#
# Env:
#   GPU_LIST       "0,1" — if unset and nvidia-smi reports 2 GPUs, uses GPUs 0 and 1
#   JOBS_PER_GPU   single int, or per-GPU list matching GPU_LIST length.
#                  Unset: 1 job per GPU.
#   EXCLUDE_GPUS   used only when auto-selecting GPUs on machines with != 2 GPUs
...
echo "======== $(date -Is) SIMSv2 tier3_fresh trial53 PRISM ablation batch: GPUs=[${GPU_IDS[*]}] JOBS_PER_GPU=[${JOBS_ARR[*]}] PARALLEL=${PARALLEL} (EXCLUDE_GPUS auto-only: ${EXCLUDE_GPUS:-∅}) ========"
echo "NOTE: Baseline none vs fixed_experiment — run none as the sole CUDA job on that GPU."

# shellcheck disable=SC2206
MODES=( ${MODES:-none no_infogate no_mselector no_ib no_conf} )
n_modes=${#MODES[@]}
...
for ((start = 0; start < n_modes; start += PARALLEL)); do
  ...
  for ((slot = 0; slot < PARALLEL; slot++)); do
    ...
    m="${MODES[$idx]}"
    if [[ "$m" == "none" ]]; then
      OUT="${MY_CREATION}/ablation_study/runs/simsv2_tier3_fresh_trial53"
    else
      OUT="${MY_CREATION}/ablation_study/runs/simsv2_tier3_fresh_trial53_${m}"
    fi
    mkdir -p "${OUT}/checkpoints"
    : >"${OUT}/train.log"
    echo "  $(date -Is) ablation=${m} CUDA_VISIBLE_DEVICES=${gpu} slot=${slot} OUT=${OUT}"
    CUDA_VISIBLE_DEVICES="${gpu}" nohup "$PYTHON" -u ablation_study/train_fixed_simsv2_tier3_fresh_trial53.py \
      --ablation "$m" \
