#!/usr/bin/env python3
"""Launch ``ablation_study/train.py`` with frozen MOSI phase4_mosi trial 234 hyperparameters.

All modes (including ``--ablation none``) use this directory's ``train.py`` and
``mosi_trial234_hparams.build_train_argv`` so code and argv stay under
...
def _default_checkpoint_dir(my_creation: Path, ablation: str) -> Path:
    ...
    if ablation == "none":
        return base / "mosi_trial234/checkpoints"
    return base / f"mosi_trial234_{ablation}/checkpoints"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="PRISM ablation: ``no_mselector``=w/o DPR, ``no_ib``=w/o VTB (same trial234 knobs otherwise).",
    )
    ap.add_argument(
        "--checkpoint-dir",
        default=None,
        help="Passed to train.py --checkpoint_dir (default: runs/mosi_trial234[_<ablation>]/checkpoints)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
