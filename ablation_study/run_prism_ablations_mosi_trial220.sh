#!/usr/bin/env bash
# MOSI trial220: six PRISM --ablation modes, separate runs/*/ dirs.
...
#
# Baseline (--ablation none) certification vs Optuna gold log
...
echo "======== $(date -Is) MOSI trial220 PRISM ablation batch: GPUs=[${GPU_IDS[*]}] JOBS_PER_GPU=[${JOBS_ARR[*]}] PARALLEL=${PARALLEL} (EXCLUDE_GPUS auto-only: ${EXCLUDE_GPUS:-∅}) ========"
...
for ((start = 0; start < n_modes; start += PARALLEL)); do
  ...
  for ((slot = 0; slot < PARALLEL; slot++)); do
    ...
    if [[ "$m" == "none" ]]; then
      ...
      OUT="${MY_CREATION}/ablation_study/runs/mosi_trial220_${m}"
    fi
    mkdir -p "${OUT}/checkpoints"
    : >"${OUT}/train.log"
    echo "  $(date -Is) ablation=${m} CUDA_VISIBLE_DEVICES=${gpu} slot=${slot} OUT=${OUT}"
    CUDA_VISIBLE_DEVICES="${gpu}" nohup "$PYTHON" -u ablation_study/train_fixed_mosi_trial220.py \
      --ablation "$m" \
