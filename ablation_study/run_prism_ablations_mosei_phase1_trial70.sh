...
# So queue scripts (and nohup wrappers) wait on this batch bash, not an outer shell PID.
echo $$ >"${MY_CREATION}/ablation_study/runs/mosei_phase1_trial70_prism_master.pid"
...
for ((start = 0; start < n_modes; start += PARALLEL)); do
  ...
  for ((slot = 0; slot < PARALLEL; slot++)); do
    ...
    m="${MODES[$idx]}"
    if [[ "$m" == "none" ]]; then
      OUT="${MY_CREATION}/ablation_study/runs/mosei_phase1_trial70"
    else
      OUT="${MY_CREATION}/ablation_study/runs/mosei_phase1_trial70_${m}"
    fi
    mkdir -p "${OUT}/checkpoints"
    : >"${OUT}/train.log"
    echo "  $(date -Is) ablation=${m} CUDA_VISIBLE_DEVICES=${gpu} slot=${slot} OUT=${OUT}"
    CUDA_VISIBLE_DEVICES="${gpu}" nohup "$PYTHON" -u ablation_study/train_fixed_mosei_phase1_trial70.py \
      --ablation "$m" \
