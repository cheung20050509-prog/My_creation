...
internal MLP head. Backbone weights are loaded via parent ``from_pretrained``
(``args.model`` / checkpoint), not nested ``DebertaV2Model.from_pretrained``.
"""
...
class InfoGate_DebertaModel(DebertaV2PreTrainedModel):
    def __init__(self, config, multimodal_config):
        ...
        # `DebertaV2Model.from_pretrained` here leaves tensors on `meta` and `.to(DEVICE)`
        # fails. Use config-only init; parent `from_pretrained` loads `dberta.model.*`.
