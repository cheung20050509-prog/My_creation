#!/usr/bin/env python3
"""Launch ``ablation_study/train.py`` with frozen MOSI phase4_mosi_space2 trial 121 hyperparameters."""
...
from mosi_space2_trial121_hparams import build_train_argv


def _default_checkpoint_dir(my_creation: Path, ablation: str) -> Path:
    base = my_creation / "ablation_study/runs"
    if ablation == "none":
        return base / "mosi_space2_trial121/checkpoints"
    return base / f"mosi_space2_trial121_{ablation}/checkpoints"
...
def main() -> int:
    ...
    ap.add_argument(
        "--checkpoint-dir",
        default=None,
        help="Passed to train.py --checkpoint_dir (default: runs/mosi_space2_trial121[_<ablation>]/checkpoints)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
