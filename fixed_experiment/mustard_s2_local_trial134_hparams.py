...
Sources (keep in sync when regenerating):
  ...
  - Best Results (dev ``binary_acc`` selection, epoch >= 13): test Acc **79.41%**, F1 **79.34%**
  - Paper: ``acl_latex.tex`` / ``mhd_msd.py`` — **79.4%** (rounded from 79.41%)

Tier-2 local search; ``gumbel_tau_*``, ``num_heads``, ``unified_dim``, and tier-3 tricks
use ``optuna_search_classify.DEFAULTS``.
"""
