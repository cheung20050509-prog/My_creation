"""Frozen hyperparameters for CH-SIMS v2 Optuna 4090D_restart phase6 trial 0.

Study ``infogate_simsv2_phase6_simsv2_4090d_space3``. Driver matches
``run_optuna_4090d_restart.sh`` ``phase6_simsv2``: ``--selection_metric mae``,
``--early_stop_patience 15``. Optuna-reported value **0.3113** (selection objective).

Sources:
  - ``logs/optuna/4090D_restart/phase6_simsv2/run/simsv2.log`` — ``Trial 0 finished`` line
  - ``logs/optuna/4090D_restart/phase6_simsv2/train_logs/simsv2_phase6_simsv2_trial_0.log`` — banner
  - ``My_creation/scripts/verify_simsv2_phase6_trial0_optuna_train_argv.py``

``batch_config`` 0 → ``BATCH_CANDIDATES["simsv2"][0]`` = (8, 4).
"""
...
TRIAL_0_PARAMS: dict[str, float | int] = {
    "n_epochs": 57,
...
IB_HIDDEN_DIM = int(TRIAL_0_PARAMS["unified_dim"])
